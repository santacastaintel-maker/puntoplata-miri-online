...
The installer for turso_cli 0.6.0

.DESCRIPTION

This script detects what platform you're on and fetches an appropriate archive from
https://github.com/tursodatabase/turso/releases/download/v0.6.0
...
$app_name = 'turso_cli'
$app_version = '0.6.0'
if ($env:TURSO_CLI_DOWNLOAD_URL) {
  $ArtifactDownloadUrls = @($env:TURSO_CLI_DOWNLOAD_URL)
} elseif ($env:INSTALLER_DOWNLOAD_URL) {
  $ArtifactDownloadUrls = @($env:INSTALLER_DOWNLOAD_URL)
} elseif ($env:TURSO_CLI_INSTALLER_GHE_BASE_URL) {
  $installer_base_url = $env:TURSO_CLI_INSTALLER_GHE_BASE_URL
  $ArtifactDownloadUrls = @("$installer_base_url/tursodatabase/turso/releases/download/v0.6.0")
} elseif ($env:TURSO_CLI_INSTALLER_GITHUB_BASE_URL) {
  $installer_base_url = $env:TURSO_CLI_INSTALLER_GITHUB_BASE_URL
  $ArtifactDownloadUrls = @("$installer_base_url/tursodatabase/turso/releases/download/v0.6.0")
} else {
  $ArtifactDownloadUrls = @("https://github.com/tursodatabase/turso/releases/download/v0.6.0")
}

$auth_token = $env:TURSO_CLI_GITHUB_TOKEN

$receipt = @"
{"binaries":["CARGO_DIST_BINS"],"binary_aliases":{},"cdylibs":["CARGO_DIST_DYLIBS"],"cstaticlibs":["CARGO_DIST_STATICLIBS"],"install_layout":"unspecified","install_prefix":"AXO_INSTALL_PREFIX","modify_path":true,"provider":{"source":"cargo-dist","version":"0.31.0"},"source":{"app_name":"turso_cli","name":"turso","owner":"tursodatabase","release_type":"github"},"version":"0.6.0"}
"@
if ($env:XDG_CONFIG_HOME) {
  $receipt_home = "${env:XDG_CONFIG_HOME}\turso_cli"
} else {
  $receipt_home = "${env:LOCALAPPDATA}\turso_cli"
}

if ($env:TURSO_CLI_DISABLE_UPDATE) {
...
if ($NoModifyPath) {
    Write-Information "-NoModifyPath has been deprecated; please set TURSO_CLI_NO_MODIFY_PATH=1 in the environment"
}

if ($env:TURSO_CLI_NO_MODIFY_PATH) {
    $NoModifyPath = $true
}

$unmanaged_install = $env:TURSO_CLI_UNMANAGED_INSTALL
...
function Install-Binary($install_args) {
  ...
  $platforms = @{
    "aarch64-pc-windows-msvc" = @{
      "artifact_name" = "turso_cli-x86_64-pc-windows-msvc.zip"
      "bins" = @("tursodb.exe")
      "libs" = @()
      "staticlibs" = @()
      "zip_ext" = ".zip"
      ...
      "aliases_json" = '{}'
      "updater" = @{
        "artifact_name" = "turso_cli-x86_64-pc-windows-msvc-update"
        "bin" = "turso_cli-x86_64-pc-windows-msvc-update"
      }
    }
...
function Download($download_url, $platforms, $arch) {
  ...
  if (($null -ne $info["updater"]) -and $install_updater) {
    ...
    $out_name = "$tmp\turso_cli-update.exe"
...
function Invoke-Installer($artifacts, $platforms) {
  ...
  if (($env:TURSO_CLI_INSTALL_DIR)) {
    $force_install_dir = $env:TURSO_CLI_INSTALL_DIR
    $install_layout = "flat"
  ...
  if (-Not $dest_dir) {
    # Install to $HOME/.turso
    $dest_dir = if (($base_dir = $HOME)) {
      Join-Path $base_dir ".turso"
    }
    $dest_dir_lib = $dest_dir
  ...
  if ($install_updater) {
    ...
    [IO.File]::WriteAllLines("$receipt_home/turso_cli-receipt.json", "$receipt", $Utf8NoBomEncoding)
  }
